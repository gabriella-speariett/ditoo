...
from .packets import build_packet, encode_uint16_little_endian
...
def _collate_frames(raw_frames: list[tuple[Image, int]]) -> list[bytes]:
    ...
    for img, duration_ms in raw_frames:
        ...
        # Truncate the number of colours to fit in a single byte (0-255)
        # 0 represents 256 colours, 1-255 represent 1-255 colours respectively
        truncated_num_of_colours = num_of_colours & 0xFF

        inner_bytes = (
            encode_uint16_little_endian(duration_ms)  # TT
        ...
        frame_parts.append(b"\xaa" + encode_uint16_little_endian(frame_length) + inner_bytes)
